...
catalog = pickle.load(open('/home/skleiman27/prospectordir/data/SAGAcatalog.p', 'rb'))
...
def build_model(objid=0, add_duste=False, add_neb=False, **extras):
    ...
    model_params['total_mass']['prior'] = priors.LogUniform(mini=1.e9, maxi=1.e12)    
    # TODO: adjust metallicity priors to be consistent with the mass; can use a mass-metallicity relation
    model_params["logzsol"]["prior"] = priors.TopHat(mini=-1.4, maxi=1.2)                            
    ...
    model_params["dust2"] = {'isfree': True, 'init':0.2, 'prior':priors.TopHat(mini=0.0, maxi=2.0)}
...
if __name__ == '__main__':
    ...
    hfile = "{0}_{1}_HOSTresult.h5".format(args.outfile, ts)
